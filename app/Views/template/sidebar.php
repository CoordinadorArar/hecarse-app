...
<aside id="sidebar" class="sidebar">
    ...
    <div class="card mb-4 rounded-3 shadow-sm text-center">
        ...
        <div class="card-body">
            <img src="<?= $imagen_usuario ?>" alt="Usuario Hecarse" class="rounded-circle my-3 imagen_usuario"
                width="100">
        ...
        <?php foreach ($sidebar as $item): ?>
            ...
            <li class="nav-item">
                <?php if (!empty($submodulosRelacionados)): ?>
                    ...
                    <a class="nav-link <?= $item['Ruta'] == $url_actual ? '' : 'collapsed' ?>" data-bs-toggle="collapse"
                        href="#submenu-<?= $item['IdModulo'] ?>" role="button" aria-expanded="false"
                        aria-controls="submenu-<?= $item['IdModulo'] ?>">
                        <i class="<?= $item['Icono'] ?>"></i>
                        <span><?= $item['Nombre'] ?></span>
                        <i class="bi bi-chevron-down ms-auto"></i>
                    </a>
                    <ul class="collapse <?= $item['Ruta'] == $url_actual ? 'show' : '' ?>"
                        id="submenu-<?= $item['IdModulo'] ?>">
                        <?php foreach ($submodulosRelacionados as $sub): ?>
                            <li>
                                <a class="nav-link <?= $sub['Ruta'] == $url_actual ? '' : 'collapsed' ?>"
                                    href="<?= base_url() . $sub['Ruta'] ?>">
                                    <i class="<?= $sub['Icono'] ?>"></i>
                                    <?= $sub['Nombre'] ?>
                                </a>
                            </li>
                ...
                <?php else: ?>
                    ...
                    <a class="nav-link <?= $item['Ruta'] == $url_actual ? '' : 'collapsed' ?>"
    ...
    <div class="mt-5 mb-2">
        <small class="text-center">&copy; 2026 Derechos reservados Hecarse.</small>
        <br><br>
        <small><i>Versión 1.0</i></small>
    </div>
</aside>
